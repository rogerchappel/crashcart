...
`inspect` accepts complete schema version 1 bundles generated by crashcart. It
validates the bundle and every nested field before rendering output. Invalid
JSON, unsupported schema versions, and malformed fields exit nonzero with a
diagnostic that names both the bundle path and the invalid field; they never
fall through to incidental JavaScript runtime errors.
...
- Bundle fields may change before a stable 1.0 release; `inspect` rejects schema
  versions it does not support.
